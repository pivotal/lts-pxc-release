...
var _ = Describe("Tar", func() {
	It("Calls tar with the correct flags", func() {
		...
		cmd := extractor.Tar("/path/to/output/directory/outputTarFile.tar", "/path/to/input/directory/")
		Expect(cmd.Args).To(Equal([]string{"something_tar_command", "cvSf", "/path/to/output/directory/outputTarFile.tar", "-C", "/path/to/input/directory/", "."}))
	})
})
